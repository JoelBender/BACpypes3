...
1. Simple Discovery (Who-Is only):
   python discover-objects-2.py --low 1 --high 3456799

2. Deep Scan (CSV + RDF + Live Table):
   python discover-objects-2.py --low 1 --high 3456799 --out-dir bacnet_to_223P --console-table
"""
...
async def get_device_object_list_robust(
    ...
    print(
        "\n    (Fallback: Reading index-by-index...)",
        end="",
        flush=True,
        file=sys.stderr,
    )
...
async def main() -> None:
    ...
    parser.add_argument(
        "--low", type=int, required=True, help="Low Device Instance Limit"
    )
    parser.add_argument(
        "--high", type=int, required=True, help="High Device Instance Limit"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Print detailed property reads errors"
    )
    ...
    parser.add_argument(
        "--console-table",
        action="store_true",
        help="Print a nice table of points to console while scanning",
    )
    ...
    parser.add_argument(
        "--filename", default="site_scan", help="Base filename for outputs"
    )
    ...
    try:
        ...
        if simple_mode:
            ...
            for i_am in i_ams:
                ...
                print(
                    f"  Device {dev_id[1]:<10} | {str(i_am.pduSource):<20} | {vendor_name}"
                )
            print("-" * 60)
            return
        ...
        print(
            f"Found {len(i_ams)} devices. Starting Deep Scan -> {out_dir} ...",
            file=sys.stderr,
        )
        ...
        for i_am in i_ams:
            ...
            try:
                ...
                print(f" [Read Property Error: {e}]", file=sys.stderr)
            ...
            if not obj_list:
                print("    ! No objects found or read failed.", file=sys.stderr)
                continue

            # Print Table Header if requested
            if show_table:
                print(
                    f"    {'Type':<20} {'Inst':<8} {'Name':<30} {'Value':<15} {'Priorities'}"
                )
            ...
            for obj_id in obj_list:
                ...
                for prop_name in props:
                    ...
                    if not obj_class.get_property_type(prop_id):
                        continue
                    ...
                    try:
                        ...
                        if prop_name == "object-name":
                            ...
                            row_data["Units"] = str(val)
                        elif prop_name == "reliability":
                            row_data["Reliability"] = str(val)
                        elif prop_name == "out-of-service":
                            row_data["OutOfService"] = 1 if val else 0
                        elif prop_name == "priority-array":
                            row_data["PriorityArray"] = format_priority_array(val)
                        ...
                        if verbose:
                            print(
                                f"      Err reading {prop_name}: {e}", file=sys.stderr
                            )
                ...
                if show_table:
                    ...
                    print(
                        f"    {str(obj_id[0]):<20} {str(obj_id[1]):<8} {display_name:<30} {display_val:<15} {row_data['PriorityArray']}"
                    )

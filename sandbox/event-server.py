...
class AnalogValueObject(_Object, _AnalogValueObject):
    _event_algorithm: Optional[OutOfRangeEventAlgorithm]
    _fault_algorithm: Optional[OutOfRangeFaultAlgorithm]
...
class EventEnrollmentObject(_Object, _EventEnrollmentObject):
    ...
    async def post_init(self):
        ...
        if event_type == EventType.outOfRange:
            self._event_algorithm = OutOfRangeEventAlgorithm(self, self._monitored_object)
        else:
        ...
        if event_type == FaultType.faultOutOfRange:
            self._fault_algorithm = OutOfRangeFaultAlgorithm(self, self._monitored_object)
        else:
...
async def main() -> None:
    global app, avo1, avo2, eeo1, avo3, eeo2
